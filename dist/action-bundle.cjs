...
async function measureImportSize(pkg) {
  ...
  return import_zlib.default.gzipSync(code).length;
}
...
function formatResultsMarkdown(results, limit) {
  const lines = [
...
function printResults(results, limit) {
  for (const r of results) {
    ...
    if (r.exceeded) {
      console.log(
        `${source_default.red("\u2717")} ${source_default.bold(r.pkg.padEnd(20))} ${source_default.red(sizeStr)}  \u2190 exceeds ${limitStr} limit!`
      );
    } else {
      console.log(
        `${source_default.green("\u2713")} ${source_default.bold(r.pkg.padEnd(20))} ${source_default.green(sizeStr)}`
      );
    }
  }
}
...
var COMMENT_MARKER = "<!-- import-cost-ci -->";
...
function buildCommentBody(results, limit) {
...
async function maybePostGitHubComment(results, limit) {
  ...
  const comments = await githubRequest(baseUrl, { method: "GET" }, token);
...
function parseLimit(raw) {
  ...
  if (unit === "kb") return Math.round(value * 1e3);
  if (unit === "mb") return Math.round(value * 1e6);
  return Math.round(value);
}
function getActionBoolean(name, fallback) {
